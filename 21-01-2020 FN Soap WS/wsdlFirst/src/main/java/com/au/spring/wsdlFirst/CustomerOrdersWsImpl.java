package com.au.spring.wsdlFirst;
...
public class CustomerOrdersWsImpl implements CustomerOrdersPortType{
	...
	public DeleteOrdersResponse deleteOrders(DeleteOrdersRequest request) {
		BigInteger customerId=request.getCustomerId();
		List<Order> orders=customerOrders.get(customerId);
		orders.removeIf(n -> n.getId() == customerId);
	...
	public UpdateOrdersResponse updateOrders(UpdateOrdersRequest request) {
		...
		Order order=request.getOrder();
		List<Order> orders=customerOrders.get(customerId);
		orders = orders.stream().filter(n -> n.getId() == customerId).map(n -> order).collect(Collectors.toList());
